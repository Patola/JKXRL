...
// glx.h pulls in X11/Xlib.h whose macros (None, Bool, Status, Window, ...)
// clash badly with the engine. Only the single translation unit that creates
// the OpenXR session (TBXR_Common.cpp) defines XR_USE_PLATFORM_XLIB and thus
// needs the XrGraphicsBindingOpenGLXlibKHR struct; everywhere else this header
// is pulled in only for the OpenGL graphics-API structs, which don't need GLX.
