// This is the only translation unit that creates the OpenXR session and so
// needs the X11/GLX graphics binding. Define this before any header (notably
// VrCommon.h -> linux/TBXR_Common.h) so the XrGraphicsBindingOpenGLXlibKHR
// struct and <GL/glx.h> are pulled in here, and nowhere else (Xlib's macros
// would otherwise pollute the whole engine).
...
// framebuffer carries a stencil buffer (stencil shadows render into it).
...
void TBXR_EnterVR( ) {
	...
	// Create the OpenXR Session using the GLX context SDL already created and
	// made current for the desktop window.
	Display* xDisplay = glXGetCurrentDisplay();
	GLXContext glxContext = glXGetCurrentContext();
	GLXDrawable glxDrawable = glXGetCurrentDrawable();

	if (xDisplay == NULL || glxContext == NULL || glxDrawable == 0) {
		ALOGE("TBXR_EnterVR: no current GLX context (display=%p context=%p drawable=%lu)\n",
			  (void*)xDisplay, (void*)glxContext, (unsigned long)glxDrawable);
		Sys_Dialog(DT_ERROR,
			"No current OpenGL/GLX context was found. JKXR's OpenXR session needs an "
			"X11 GLX context - please run under X11 (or XWayland), not native Wayland.",
			"OpenXR GLX binding failed");
		exit(1);
	}

	// The OpenXR runtime needs the GLXFBConfig and X visual the context was
	// created with so it can create matching contexts of its own. Recover them
	// from the context's framebuffer-config id.
	...
	int nConfigs = 0;
	GLXFBConfig* fbConfigs = glXGetFBConfigs(xDisplay, DefaultScreen(xDisplay), &nConfigs);
	for (int i = 0; i < nConfigs; i++) {
		int id = 0;
		glXGetFBConfigAttrib(xDisplay, fbConfigs[i], GLX_FBCONFIG_ID, &id);
		if (id == fbConfigId) {
			glxFBConfig = fbConfigs[i];
			int vid = 0;
			glXGetFBConfigAttrib(xDisplay, fbConfigs[i], GLX_VISUAL_ID, &vid);
			visualid = (uint32_t)vid;
			break;
		}
	}
	if (fbConfigs) {
	...
	graphicsBindingGL.type = XR_TYPE_GRAPHICS_BINDING_OPENGL_XLIB_KHR;
	graphicsBindingGL.next = NULL;
	graphicsBindingGL.xDisplay = xDisplay;
	graphicsBindingGL.visualid = visualid;
	graphicsBindingGL.glxFBConfig = glxFBConfig;
	graphicsBindingGL.glxDrawable = glxDrawable;
	graphicsBindingGL.glxContext = glxContext;
